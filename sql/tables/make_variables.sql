-- I wonder why data_type is in public and not in lte_data.
-- Corrected to point to data_type_term_id which is the correct one to use (2026-03-06 - NC) 
...
create table lte_data.variables (
    ...
    data_type_term_id int2 not null,
    ...
    constraint variables_measured_property_term_id_fk foreign key (measured_property_term_id) references lte_data.measured_property_terms(id),
    constraint variables_measured_entity_term_id_fk foreign key (measured_entity_term_id) references lte_data.measured_entity_terms(id),
    constraint variables_measured_entity_part_term_id_fk foreign key (measured_entity_part_term_id) references lte_data.measured_entity_part_terms(id),
    constraint variables_data_type_term_id_fk foreign key (data_type_term_id) references lte_data.data_type_terms (id),
	constraint variables_measurement_unit_term_id_fk foreign key (measurement_unit_term_id) references lte_data.measurement_unit_terms(id)
);
...
comment on column lte_data.variables.data_type_term_id is 'An FK to identify the data type for the variable';
